...
import type { ColumnInfo, FilterCondition, FilterOperator } from './ApisPage.types';
...
export default function FilterSection({
    ...
    const handleColumnSelect = (columnName: string | null) => {
        setPendingFilter({ column: columnName || undefined, operator: undefined, value: '', value2: '' });
    };
    ...
    const canAddFilter = useMemo(() => {
        ...
        if (pendingFilter.operator === 'isNull' || pendingFilter.operator === 'isNotNull') return true;
        if (!pendingFilter.value) return false;
        if (pendingFilter.operator === 'between' && !pendingFilter.value2) return false;
        return true;
    }, [pendingFilter]);
    ...
    const addFilter = () => {
        if (canAddFilter) {
            if (editingFilterIndex !== null) {
                ...
                const newFilters = filters.map((f, i) => i === editingFilterIndex ? pendingFilter as FilterCondition : f);
    ...
    const renderValueInput = () => {
        ...
        if (pendingFilter.operator === 'isNull' || pendingFilter.operator === 'isNotNull') return null;
        ...
        if (colType === 'boolean') {
            return (
                ...
                >
                    <MenuItem value="" disabled><em>Value</em></MenuItem>
        ...
        const inputType = colType === 'number' ? 'number' :
            colType === 'date' ? 'date' :
                colType === 'datetime' ? 'datetime-local' : 'text';
        ...
        return (
            <>
                <ParameterInput
                    ...
                    value={pendingFilter.value || ''}
                    onChange={(e) => handleValueChange(e.target.value)}
                    style={{ width: pendingFilter.operator === 'between' ? '90px' : '120px' }}
                />
        ...
        <TryItSection sx={{ py: 1.5 }}>
                    ...
                    {filters.map((filter, index) => (
                        <Box
                            ...
                            sx={{
                                ...
                                py: 0.5,
                                px: 0.75,
                                bgcolor: editingFilterIndex === index ? 'primary.main' : 'action.hover',
                                color: editingFilterIndex === index ? 'primary.contrastText' : 'inherit',
                                borderRadius: 1,
                                fontSize: '0.75rem',
                        ...
                        >
                            ...
                            <Box sx={{ fontWeight: 500, color: editingFilterIndex === index ? 'inherit' : 'primary.main' }}>{getOperatorLabel(filter.operator)}</Box>
                            {filter.operator !== 'isNull' && filter.operator !== 'isNotNull' && (
                                <Box sx={{ fontFamily: 'monospace', color: editingFilterIndex === index ? 'inherit' : 'success.main' }}>
                                    {filter.value}
                            ...
                            <Box sx={{ ml: 'auto', display: 'flex', gap: 0.5 }}>
                                <Box
                                    ...
                                    sx={{
                                        cursor: 'pointer',
                                        color: editingFilterIndex === index ? 'inherit' : 'text.disabled',
                                        '&:hover': { color: editingFilterIndex === index ? 'inherit' : 'primary.main' },
                                        display: 'flex'
                                    }}
                                >
                                ...
                                <Box
                                    ...
                                    sx={{
                                        cursor: 'pointer',
                                        color: editingFilterIndex === index ? 'inherit' : 'text.disabled',
                                        '&:hover': { color: editingFilterIndex === index ? 'inherit' : 'error.main' },
                                        display: 'flex'
                                    }}
                                >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexWrap: 'wrap' }}>
                <TextField
                    ...
                    SelectProps={{ displayEmpty: true }}
                >
                    <MenuItem value="" disabled><em>Column</em></MenuItem>
                    {columns.map(c => (
                ...
                <TextField
                    ...
                    SelectProps={{ displayEmpty: true }}
                >
                    <MenuItem value="" disabled><em>Operator</em></MenuItem>
                    {availableOperators.map(op => (
                        <MenuItem key={op.value} value={op.value} sx={{ fontSize: '0.8rem' }}>{op.label}</MenuItem>
                    ))}
                </TextField>
                ...
                <AddFilterButton
                    ...
                    sx={{
                        ...
                        '&:hover': editingFilterIndex !== null ? { bgcolor: 'success.dark' } : undefined
                    }}
                >
