...
import { useSaveQuery } from '../../../../api/entities/schema/useSavedQueries';
...
export default function VisualQueryBuilder({
  ...
  const handleTableDragStart = useCallback((_table: SchemaTable) => {
  ...
  // Save query mutation
  const { mutate: saveQuery } = useSaveQuery(databaseId || 0, {
    onSuccess: () => {
  ...
  const handleSaveApiConfirm = useCallback(
    (name: string, description: string, method: string, isPublic: boolean) => {
    ...
    [databaseId, generatedSQL, saveQuery]
  );
