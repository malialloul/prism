...
export default function DatabaseActionsPanel({
  ...
  // Track autoConnect values for success handlers (using refs to avoid stale closure issues)
  const createAutoConnectRef = useRef(true);
  const connectAutoConnectRef = useRef(true);
  ...
  const { mutate: submitConnectDatabase, isPending: isConnecting } = useConnectDatabase({
    onSuccess: (database, message) => {
      ...
      if (connectAutoConnectRef.current) {
        onDatabaseConnected?.(database.id);
      }
    },
    // Error toast is handled by httpClient interceptor
  ...
  const createFormik = useFormik({
    ...
    onSubmit: async (values) => {
      createAutoConnectRef.current = values.autoConnect;
  ...
  const connectFormik = useFormik({
    ...
    onSubmit: async (values) => {
      connectAutoConnectRef.current = values.autoConnect;
      const encryptedPassword = await encryptForTransmission(values.password);
        ...
        {canCreateDatabase ? (
          <form onSubmit={createFormik.handleSubmit}>
            ...
            <DialogContent>
              ...
              <AutoConnectToggle>
                <AutoConnectLabel>
                  <AutoConnectTitle>Auto Connect</AutoConnectTitle>
                  <AutoConnectDescription>Connect to database after creation</AutoConnectDescription>
                </AutoConnectLabel>
                <Switch
                  name="autoConnect"
                  checked={createFormik.values.autoConnect}
                  onChange={createFormik.handleChange}
                  color="primary"
                />
              </AutoConnectToggle>
          ...
          <form onSubmit={connectFormik.handleSubmit}>
            ...
            <DialogContent>
              ...
              <AutoConnectToggle>
                <AutoConnectLabel>
                  <AutoConnectTitle>Auto Connect</AutoConnectTitle>
                  <AutoConnectDescription>Connect to database after saving</AutoConnectDescription>
                </AutoConnectLabel>
                <Switch
                  name="autoConnect"
                  checked={connectFormik.values.autoConnect}
                  onChange={connectFormik.handleChange}
                  color="primary"
                />
              </AutoConnectToggle>
